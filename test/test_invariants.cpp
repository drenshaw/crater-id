...
//   bool success_ab = invariants::IntersectionLines(locusA, locusB, gh_ij);
//   EXPECT_TRUE(success_ab);
//   bool success_bc = invariants::IntersectionLines(locusB, locusC, gh_jk);
//   EXPECT_TRUE(success_bc);
//   bool success_ca = invariants::IntersectionLines(locusC, locusA, gh_ki);
...
TEST(InvariantTest, InvariantTriad) {
  ...
  if(!conicA.chooseIntersection(conicB, lij)) {
    std::cerr<<"IntersectionLines error ij\n";
  }
  if(!conicA.chooseIntersection(conicC, lik)) {
    std::cerr<<"IntersectionLines error jk\n";
  }
  if(!conicA.chooseIntersection(conicD, lil)) {
    std::cerr<<"IntersectionLines error ki\n";
  }
  if(!conicB.chooseIntersection(conicC, ljk)) {
    std::cerr<<"IntersectionLines error ki\n";
  }
  if(!conicB.chooseIntersection(conicD, ljl)) {
    std::cerr<<"IntersectionLines error ki\n";
  }
  if(!conicC.chooseIntersection(conicD, lkl)) {
    std::cerr<<"IntersectionLines error ki\n";
  }
  // Invariants
  ...
  // TODO: These should be exactly the same; find out why they aren't perfect
  EXPECT_NEAR(invA, invariantsABC.at(0), 1e-8);
  EXPECT_NEAR(invB, invariantsABC.at(1), 1e-8);
...
TEST(ConicTest, ConicIntersection) {
  ...
  bool success = conicA.intersectsConicLines(conicB, gh);
